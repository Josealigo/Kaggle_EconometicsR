...
parms_sv = expand.grid("gamma"=gamma,"kernel"=kernel,"cost"=cost,"type"=type,"epsion"=epsion,"nu"=nu)
...
for (i in 1:nrow(conjunto_iterador)) {
  ...
  set.seed(1)
  svm_piv = svm(median_house_value~.,tcy,scale=T,cachesize = 1500,gamma = conjunto_iterador$gamma[i],kernel = conjunto_iterador$kernel[i], cost = conjunto_iterador$cost[i], type = conjunto_iterador$type[i],epsion=conjunto_iterador$epsion[i],nu=conjunto_iterador$nu[i])
...
(proc.time() - ptm)[3]
save.image("C:/Users/jose-/Documents/Maestria/Econometria_R/Proyecto/Kaggle_EconometicsR/Envs/SVM_3.RData")
```
...
```{r}
  ...
  set.seed(1)
  sv_final =  svm(median_house_value~.,tcy,scale=T,cachesize = 1500,gamma = conjunto_iterador$gamma[best_parm],kernel = conjunto_iterador$kernel[best_parm], cost = conjunto_iterador$cost[best_parm], type = conjunto_iterador$type[best_parm],epsion=conjunto_iterador$epsion[best_parm],nu=conjunto_iterador$nu[best_parm])
  ...
  submission2 = as.data.frame(cbind("id"=test$id,"median_house_value"=pred_f))
  ...
  write.csv(submission2,"C:/Users/jose-/Documents/Maestria/Econometria_R/Proyecto/Kaggle_EconometicsR/submission2_20200625.csv",row.names = F)
```
